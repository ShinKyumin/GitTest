...
data class Item(val id: String, val name: String, val price: Int, val stock: Boolean) {
    constructor(doc: QueryDocumentSnapshot) :
            this(doc.id, doc["name"].toString(), doc["price"].toString().toIntOrNull() ?: 0, doc["instock"] as Boolean
            )
    constructor(key: String, map: Map<*, *>) :
            this(key, map["name"].toString(), map["price"].toString().toIntOrNull() ?: 0, map["stock"] as Boolean)
}
...
class MyAdapter(private val context: Context, private var items: List<Item>)
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.binding.textStock.text = item.stock.toString()
        ...
        holder.binding.textStock.setOnClickListener {
            itemClickListener?.onItemClick(item.id)
        }
    }
